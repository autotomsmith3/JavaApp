...
public class compareBS_text_outPutAcodeName_Search_Features_Count {
	...
	public static void PostText(String env, String client, String envClientTextURL, String headers[],
		...
		try {
			prop.load(compareBS_text_outPutAcodeName_Search_Features_Count.class.getClassLoader()
...
//		String getMakeModelURL = envURL;// + VehicleSetCode + "/" + LngCode + "/" + CountryCode + "/" + year;
		...
		for (String Text_Body : TextBodys) {
			count++;
			String jsonTextFrPostTextWS = com_libs.getNewSourceCodeJsonPostTextOutPutAcodeName(env, client, Text_Body,
...
//				}
				...
				if (containsIgnoreCase(jsonTextFrPostTextWS, item)) {
					String saveFoundString = client + " - " + count + " - " + "SearchItem=\"" + item + "\"";// , exists
																											// in WS
																											// return
																											// for
																											// Text_Body:
...
//				System.out.println(saveFoundString);
					...
					int feature_count = 0, fromIndex = 0;
					while ((fromIndex = jsonTextFrPostTextWS.indexOf(item, fromIndex)) != -1) {
					...
					System.out.println("Total occurrences: " + feature_count);
					// save to file
					com_libs.SaveScratch(searchItemsfilePath_return,
							saveFoundString + "feature_count=" + feature_count);
				}
