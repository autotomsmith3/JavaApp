...
public class compareBS_vehicles_compare_to_previous {
	...
	public static void GetVehicles(String env, String client, String envClientTextURL, String headers[],
			String vehiclesCodes[], String preDateFolder, String currentDateFolder) throws Exception {
		...
		try {
			prop.load(compareBS_vehicles_compare_to_previous.class.getClassLoader()
...
//		String commonCompetitorsCodes[] = comlibs.loadTextFromDataFolder("empty", "./compareBS_data/" + client + "CommonCompetitors.txt");
		...
		for (String vehiclesCode : vehiclesCodes) {
			count++;
			String jsonVehiclesToComparesWS = com_libs.getNewSourceCodeJsonGETvehiclesToCompare(env, client,
					vehiclesCode, envURL, vehiclesCode, "vehicles", count, headers[0], headers[1], headers[2],
					headers[3], preDateFolder, currentDateFolder);
...
//		String [] commonCompetitorsCodes=new String [500] ;
		...
		for (String client : Clients) {
			...
			GetVehicles(env, client, VehiclesURL, Headers, vehiclesCodes, preDateFolder, currentDateFolder);
		}
