...
public class compareBS_commonCompetitors_compare_to_previous {
	...
	public static void GetCommonCompetitors(String env, String client, String envClientTextURL, String headers[],
			String commonCompetitorsCodes[], String preDateFolder, String currentDateFolder) throws Exception {
		...
		try {
			prop.load(compareBS_commonCompetitors_compare_to_previous.class.getClassLoader()
...
//		String commonCompetitorsCodes[] = comlibs.loadTextFromDataFolder("empty", "./compareBS_data/" + client + "CommonCompetitors.txt");
		...
		for (String commonCompetitorsCode : commonCompetitorsCodes) {
			count++;
			String jsonCommonCompetitorToComparesWS = com_libs.getNewSourceCodeJsonGETcommonCompetitorsToCompare(env,
					client, commonCompetitorsCode, envURL, commonCompetitorsCode, "common-competitors", count,
					headers[0], headers[1], headers[2], headers[3], preDateFolder, currentDateFolder);
...
//		String [] commonCompetitorsCodes=new String [500] ;
		...
		for (String client : Clients) {
			...
			GetCommonCompetitors(env, client, CommonCompetitorsURL, Headers, commonCompetitorsCodes, preDateFolder,
					currentDateFolder);
		}
